...
st.markdown(
    ...
    <style>
        ...
        .badge-low { background-color: #e3f5e6; color: #1a7a34; }
        .badge-medium { background-color: #fff4dd; color: #9a6a00; }
        .badge-high { background-color: #ffe9de; color: #b8471a; }
        .badge-urgent { background-color: #fde2e2; color: #c22a2a; }
        .badge-open { background-color: #e3edfd; color: #1c5cc4; }
        .badge-accepted { background-color: #e3f5e6; color: #1a7a34; }
        .badge-discarded { background-color: #f1f1f3; color: #55575c; }
        .badge-failed { background-color: #fde2e2; color: #c22a2a; }
        .badge-pending { background-color: #f1f1f3; color: #55575c; }
        .claim-card-label {
            font-size: 0.78rem;
...
def _badge(text: str, kind: str) -> str:
    css_class = f"badge-{kind.lower()}" if kind else "badge-pending"
    return f'<span class="badge {css_class}">{text}</span>'


st.markdown(
    '<div class="claims-copilot-hero"><h1>🛡️ Insurance Claims Copilot Workbench</h1></div>',
    unsafe_allow_html=True,
)
st.markdown(
    '<div class="claims-copilot-subtitle">Register FNOL claims, generate AI-assisted '
    "coverage recommendations, and review claim history in one place.</div>",
    unsafe_allow_html=True,
)
...
def render_context(context: dict[str, Any] | None) -> None:
    ...
    c3.metric("🛠️ Decision Tool Calls", signals.get("tool_call_count", len(tool_calls)))
    c4.metric(
        "⚠️ Tool Errors",
        signals.get(
            "tool_error_count",
    ...
    if tool_calls:
        ...
        for index, call in enumerate(tool_calls, start=1):
            status_icon = "✅" if call.get("status") == "ok" else "❌"
            title = (
                f"{status_icon} {index}. {call.get('tool_name', 'unknown')} "
    ...
    if errors:
        with st.expander("⚠️ Context Errors"):
            for err in errors:
                st.error(err)
...
with st.sidebar:
    ...
    if st.button("Ingest Policy & Regulation KB", use_container_width=True, type="primary"):
        with st.spinner("Indexing policy & regulation documents..."):
            try:
    ...
    st.divider()
    st.caption("Insurance Claims Copilot · AI-assisted, adjuster-approved.")


# --------------------------------------------------------------------------
# Main content — tabbed layout
# --------------------------------------------------------------------------
tab_register, tab_claims = st.tabs(["📝 Register Claim (FNOL)", "📂 Claims"])
    ...
    with st.form("create_ticket_form"):
        ...
        with col1:
            ...
            customer_company = st.text_input(
                "Insured Organization (Optional)", placeholder="Acme Logistics"
            )
        ...
        submitted = st.form_submit_button("Register Claim", type="primary", use_container_width=True)
        if submitted:
...
with tab_claims:
    ...
    if not tickets:
        st.info("No claims yet. Register one in the **Register Claim** tab.")
    else:
        m1, m2, m3 = st.columns(3)
        m1.metric("Total Claims", len(tickets))
        m2.metric("Urgent/High", len([t for t in tickets if t["priority"] in ("high", "urgent")]))
        m3.metric("Open", len([t for t in tickets if t["status"] == "open"]))

        st.markdown("#### Select a Claim")
        labels = [
            f"#{t['id']} · {t['status']} · {t['customer_email']} · {t['subject']}"
            for t in tickets
        ]
        selected_label = st.selectbox("Claim", labels, label_visibility="collapsed")
        ...
        with c1:
            st.markdown('<div class="claim-card-label">Claimant</div>', unsafe_allow_html=True)
            st.markdown(f"**{selected_ticket['customer_email']}**")
            st.write(selected_ticket.get("customer_name") or "-")
            st.write(selected_ticket.get("customer_company") or "-")

        with c2:
            st.markdown('<div class="claim-card-label">Claim</div>', unsafe_allow_html=True)
            st.markdown(
                _badge(selected_ticket["priority"].upper(), selected_ticket["priority"])
                + " "
                + _badge(selected_ticket["status"].upper(), selected_ticket["status"]),
                unsafe_allow_html=True,
            )
        ...
        if st.button("✨ Generate Coverage Recommendation", use_container_width=True, type="primary"):
            with st.spinner("Generating coverage recommendation..."):
                try:
        ...
        if draft_data:
            ...
            st.caption("⚖️ AI provides recommendation; licensed adjuster makes final decision.")
            ...
            with c4:
                if st.button("ℹ️ Request Info", use_container_width=True):
                    try:
        ...
        if st.button("Run Claim History Probe", use_container_width=True):
            with st.spinner("Searching claim history..."):
                try:
